...
class MAResult:
    ...
    def __init__(self, df_trades, pair_name, ma_l, ma_s, granularity) -> None:
...
def get_full_name(filepath, filename):
...
def process_results(results_list, filepath):
    process_macro(results_list, get_full_name(filepath, "ma_res"))
    process_trades(results_list, get_full_name(filepath, "ma_trades"))
    # rl = [x.result for x in results_list]
    # df = pd.DataFrame.from_dict(rl)
    # print(df)
...
def run_ma_sim(curr_list=["EUR", "USD"], granularities=["H1", "H4"], ma_long=[20, 40, 80, 120, 150], ma_short=[10, 20, 30, 40], filepath="./data"):
        ...
        for p1 in curr_list:
            for p2 in curr_list:
                ...
                if pair in ic.instruments_dict.keys():
                    analyze_pair(ic.instruments_dict[pair], g, ma_long, ma_short, filepath)